...
class PremiumUpgradePlaceholderPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PremiumUpgradePlaceholderPage> createState() =>
      _PremiumUpgradePlaceholderPageState();
}

class _PremiumUpgradePlaceholderPageState
    extends ConsumerState<PremiumUpgradePlaceholderPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final monthlyPrice = pricing.valueOrNull?.firstWhere(
            (e) => e['name'] == 'monthly',
            orElse: () => {
                  'price_idr': SubscriptionConfig.monthlyPriceIdr
                })['price_idr'] as int? ??
        SubscriptionConfig.monthlyPriceIdr;
    final yearlyPrice = pricing.valueOrNull?.firstWhere(
            (e) => e['name'] == 'yearly',
            orElse: () => {
                  'price_idr': SubscriptionConfig.yearlyPriceIdr
                })['price_idr'] as int? ??
        SubscriptionConfig.yearlyPriceIdr;
    const benefits = [
              ...
              GlassCard(
                ...
                child: Column(
                  children: [
                    ...
                    Text('Your Sound, Without Limits.',
                        style: Theme.of(context)
                            .textTheme
                            .titleLarge
                      ...
                      children: [
                        Expanded(
                            child: _PlanCard(
                                label: 'Monthly',
                                price:
                                    '${SubscriptionConfig.formatIdr(monthlyPrice)}/bulan',
                        ...
                        Expanded(
                            child: _PlanCard(
                                label: 'Yearly',
                                price:
                                    '${SubscriptionConfig.formatIdr(yearlyPrice)}/tahun\nHemat 24%',
                        ...
                        child: CupertinoButton.filled(
                            ...
                            child: _loading
                                ? const CupertinoActivityIndicator(
                                    color: Colors.white)
                                : Text(
                                    'Continue — ${SubscriptionConfig.formatIdr(_plan == 'monthly' ? monthlyPrice : yearlyPrice)}'))),
                    const SizedBox(height: 8),
                    Text('Midtrans Sandbox — no real charge',
                        style: TextStyle(
                    ...
                    Consumer(
                      builder: (context, ref, _) {
                        ...
                        return Column(
                          children: [
                            Text(
                                'Active: ${active.planType} until ${active.endDate?.toLocal().toString().split(' ').first ?? '-'}',
                                ...
                                final ok = await showCupertinoDialog<bool>(
                                    ...
                                    builder: (c) => CupertinoAlertDialog(
                                            title: const Text(
                                                'Cancel subscription?'),
                                            content: const Text(
                                                'You will remain Premium until end_date, then downgrade to Free.'),
                                            actions: [
                                    ...
                                    if (context.mounted)
                                      ScaffoldMessenger.of(context)
                                          .showSnackBar(const SnackBar(
                                              content: Text(
                                                  'Cancelled — remains Premium until expiry')));
                                  } catch (e) {
                                    if (context.mounted)
                                      ScaffoldMessenger.of(context)
        ...
        child: Column(children: [
          Text(label,
              style: TextStyle(
                  ...
                  color: selected ? AppColors.azureMistDeep : null)),
