...
class _DownloadButtonState extends ConsumerState<DownloadButton> {
  ...
  Future<void> _check() async {
    final v = await OfflineDownloadService.instance.isDownloaded(widget.song.id);
  ...
  Widget build(BuildContext context) {
    ...
    if (gate.shouldShowAds) {
      ...
      return IconButton(
        ...
        onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Download hanya untuk Premium'))),
      );
    }
    if (_isDownloaded == null) return const SizedBox(width: 24, height: 24, child: CupertinoActivityIndicator());
    if (_isDownloaded == true) {
      return IconButton(icon: const Icon(CupertinoIcons.checkmark_alt_circle_fill, color: Colors.green), onPressed: () async { await OfflineDownloadService.instance.delete(widget.song.id); if (mounted) setState(() => _isDownloaded = false); });
    }
    return IconButton(
      icon: _downloading ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(CupertinoIcons.cloud_download),
      onPressed: _downloading
          ...
          : () async {
              ...
              try {
                ...
                final url = await r2.getReadUrl(widget.song.audioUrl!, expires: 3600);
                await OfflineDownloadService.instance.downloadFromUrl(widget.song.id, url);
