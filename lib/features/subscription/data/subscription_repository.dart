...
class SubscriptionRepository {
  ...
  Future<Map<String, dynamic>> requestSnapToken(
      {required String planType}) async {
    final res = await _supabase.functions
        .invoke('create-snap-token', body: {'plan_type': planType});
    if (res.status != 200)
      throw Exception(res.data?['error'] ?? 'Failed to create snap token');
