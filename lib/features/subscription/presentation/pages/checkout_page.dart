...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  void initState() {
    ...
    if (kIsWeb) {
      if (widget.redirectUrl != null)
        launchUrl(Uri.parse(widget.redirectUrl!),
            ...
            onNavigationRequest: (req) {
              ...
              if (url.contains('payment/finish') ||
                ...
                if (!_handled) {
                  ...
                  var outcome = 'pending';
                  if (url.contains('settlement') || url.contains('capture'))
                    outcome = 'success';
                  if (url.contains('deny') ||
                      url.contains('failure') ||
                      url.contains('cancel')) outcome = 'failure';
  ...
  Widget build(BuildContext context) {
      ...
      return Scaffold(
          ...
          body: Center(
              child: Column(
                  ...
                  children: [
                const Text('Redirecting to Midtrans Snap...'),
                const SizedBox(height: 12),
                if (widget.redirectUrl != null)
                  ElevatedButton(
                      onPressed: () => launchUrl(Uri.parse(widget.redirectUrl!),
                          mode: LaunchMode.externalApplication),
    ...
    if (_ctrl == null)
      return Scaffold(
          appBar: AppBar(title: const Text('Checkout')),
          body: const Center(child: Text('No snap token')));
    return Scaffold(
        appBar: AppBar(title: Text('Checkout - ${widget.plan}')),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                  isSuccess
                      ? 'Premium akan aktif setelah webhook memverifikasi.'
                      : isPending
