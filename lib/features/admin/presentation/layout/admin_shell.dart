...
class AdminShell extends ConsumerWidget {
      ...
      body: Row(children: [
        Container(
          ...
          child: SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
            Text('Rakyzu Admin', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800, color: AppColors.azureMistDeep)),
            Text('Role: ${role?.value ?? '-'}', style: const TextStyle(color: AppColors.textSecondary, fontSize:12)),
            const Divider(height:32),
            _nav(context, Icons.dashboard, 'Analytics', '/admin/analytics', enabled: canAnalytics),
            _nav(context, Icons.attach_money, 'Revenue', '/admin/revenue', enabled: canRevenue),
            _nav(context, Icons.people, 'Users', '/admin/users', enabled: canUsers),
            _nav(context, Icons.library_music, 'Catalog', '/admin/catalog', enabled: canCatalog),
            _nav(context, Icons.price_change, 'Pricing', '/admin/pricing', enabled: canPricing),
  ...
  Widget _nav(BuildContext ctx, IconData icon, String label, String route, {required bool enabled}) {
    ...
    return ListTile(
      enabled: enabled,
      leading: Icon(icon, color: enabled ? (selected? AppColors.azureMistDeep: AppColors.textSecondary) : Colors.grey),
      title: Text(label, style: TextStyle(color: enabled ? (selected? AppColors.azureMistDeep: null) : Colors.grey, fontWeight: selected?FontWeight.w700:null)),
