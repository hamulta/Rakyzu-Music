      ...
      children: [
        ...
        users.when(
            data: (m) => GlassCard(
                ...
                child: Column(
                    ...
                    children: [
                      const Text('Total Users per Role',
        ...
        Row(children: [
          ...
          IconButton(
              ...
              onPressed: () async {
                final data = await ref.read(adminTopSongsProvider.future);
                final csv = toCsv(data);
                await Share.share('Top Songs CSV\n$csv');
              })
        ]),
        const SizedBox(height: 8),
        top.when(
                ...
                child: Column(
                    children: list
                        ...
                        .map((e) => ListTile(
                            ...
                            subtitle: Text(
                                'artist: ${e.value['artist']?['name'] ?? '-'}'),
                            trailing:
                                Text('${e.value['play_count'] ?? 0} plays')))
                        .toList())),
            loading: () => const CupertinoActivityIndicator(),
            error: (e, _) => Text('Error $e')),
      ],
