...
class _PricingPageState extends ConsumerState<PricingPage> {
  ...
  bool _saving=false;
  @override
  void dispose(){ _monthly.dispose(); _yearly.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context){
    final pricing = ref.watch(adminPricingProvider);
    return ListView(padding: const EdgeInsets.all(20), children: [
      Text('Pricing Plans (Owner Only)', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800)),
      const SizedBox(height:8), const Text('Monthly/Yearly harga yang sebelumnya hardcoded di v0.7.1 kini data-driven.', style: TextStyle(color: Colors.grey, fontSize:12)),
      const SizedBox(height:16),
      pricing.when(data: (list){
        final monthly = list.firstWhere((e)=> e['name']=='monthly', orElse:()=> {'price_idr':49000});
        final yearly = list.firstWhere((e)=> e['name']=='yearly', orElse:()=> {'price_idr':449000});
        _monthly.text = _monthly.text.isEmpty ? '${monthly['price_idr']}' : _monthly.text;
        _yearly.text = _yearly.text.isEmpty ? '${yearly['price_idr']}' : _yearly.text;
        return GlassCard(padding: const EdgeInsets.all(16), child: Column(children: [
          TextField(controller: _monthly, decoration: const InputDecoration(labelText: 'Monthly Price IDR'), keyboardType: TextInputType.number),
